...
void tyran_mocha_parser_enclosure_stack_push(tyran_mocha_parser_enclosure_stack* stack, NODE* root, tyran_mocha_token_id start_enclosure_id)
{
...
NODE* tyran_mocha_parser_last_node_helper(NODE* node)
{
	tyran_parser_node_operand_binary* binary = tyran_parser_binary_operator_cast(*node);
	if (binary) {
		return tyran_mocha_parser_last_node_helper(&binary->left);
	}
...
{
	return tyran_mocha_parser_last_node_helper(parser->root);
}
...
void tyran_mocha_parser_push_root_right(tyran_mocha_parser* parser, tyran_parser_node_operand_binary* node)
{
	...
	tyran_parser_node_print("PUSH ROOT RIGHT NODE:", parser->root, &print_node);
	tyran_mocha_parser_node_print_tree(parser, "BEFORE PUSH RIGHT", 0);
	tyran_mocha_parser_push_right(parser, node, actual_root);
	tyran_mocha_parser_node_print_tree(parser, "AFTER PUSH RIGHT", 0);
}
...
void tyran_mocha_parser_push_last_inserted_right(tyran_mocha_parser* parser, tyran_parser_node_operand_binary* node)
{
	...
	tyran_mocha_parser_node_print_tree(parser, "BEFORE PUSH LAST RIGHT", 0);
...
NODE tyran_mocha_parser_add_default_operator(tyran_memory* memory, tyran_mocha_parser* parser, tyran_mocha_token_id token_id, int precedence)
{
	if (tyran_mocha_lexer_is_unary_operator(token_id)) {
		...
		TYRAN_LOG("Compare precedence %d  %d", precedence, parser->overwrite_precedence);
		if (precedence < parser->overwrite_precedence) {
			tyran_mocha_parser_push_root_right(parser, node);
		} else {
			tyran_mocha_parser_push_last_inserted_right(parser, node);
		}
		parser->overwrite_precedence = precedence;
		return (NODE)node;
	}
}

NODE tyran_mocha_parser_if(tyran_memory* memory, tyran_mocha_parser* parser)
{
	...
	if (else_block && else_block->type == TYRAN_PARSER_NODE_TYPE_ELSE) {
		...
		return tyran_parser_if_else(memory, expression, block, ((tyran_parser_node_else*)else_block)->block);
	} else {
		return tyran_parser_if(memory, expression, block);
	}
}
...
NODE tyran_mocha_parser_add_terminal(tyran_memory* memory, tyran_mocha_parser* parser, tyran_mocha_token_id token_id, int precedence)
{
	...
	switch (token_id) {
		case TYRAN_MOCHA_TOKEN_IF:
			node = tyran_mocha_parser_if(memory, parser);
			break;
		case TYRAN_MOCHA_TOKEN_ELSE:
...
void tyran_mocha_parser_add_enclosure(tyran_mocha_parser* parser, tyran_parser_node_operand_unary* enclosure_node, tyran_mocha_token_id end_closure, tyran_mocha_token_id start_closure)
{
	...
	tyran_mocha_parser_enclosure_stack_push(parser->enclosure_stack, parser->root, parser->waiting_for_start_enclosure_id);
