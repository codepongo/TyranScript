...
int tyran_mocha_lexer_operand(tyran_lexer* lexer, int c)
{
	...
	static tyran_operand_info operands[] = {
		...
		{ "<=", 1, TYRAN_MOCHA_TOKEN_LESS_EQUAL },
		{ ">=", 1, TYRAN_MOCHA_TOKEN_GREATER_EQUAL },
...
{
	return (token_id == TYRAN_MOCHA_TOKEN_IF || token_id == TYRAN_MOCHA_TOKEN_PARENTHESES_LEFT || token_id == TYRAN_MOCHA_TOKEN_BLOCK_START);
}
